...
public class IMConnection {
	 ...
	 */
	public boolean connect(String option) {
		...
		boolean retVal = checkForCrudOperation(option);
	...
	public void sendDupingMessage(String message) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		Message bctMessage = Message.makeDupingMessage(userName, message);
		socketConnection.print(bctMessage);
	}
	...
	public void sendUpdateMessage(String message) {
		...
		Message updMessage = Message.makeUpdateMessage(userName, message);
		socketConnection.print(updMessage);
	}
	...
	public void sendDeleteMessage() {
		...
		Message updMessage = Message.makeDeleteMessage(userName);
		socketConnection.print(updMessage);
	}
	
	public void sendPrivateMessage(String rec, String message) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		
		Message pvtMessage = Message.makePrivateMessage(userName, rec, message);
		socketConnection.print(pvtMessage);
	}
	

	public void sendGroupAddMessage(String groupName, String users) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		Message groupAdd = Message.makeGroupAdd(userName,groupName, users);
		
		socketConnection.print(groupAdd);
	}

	public void sendGroupMessage(String grp, String message) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		
		Message grpMessage = Message.makeGroupMessage(userName, grp, message);
		socketConnection.print(grpMessage);
	}
	
	public void sendGroupUpdateMessage(String groupName, String users) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		
		Message groupUpdate = Message.makeGroupUpdate(userName,groupName, users);
		socketConnection.print(groupUpdate);
	}
	
	public void sendRecallMessage(String message) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		Message rclMessage = Message.makeRecallMessage(userName, message);
		socketConnection.print(rclMessage);
	}
	
	public void sendGroupDeleteMessage(String groupName) {
		if (!connectionActive()) {
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		
		Message groupDelete = Message.makeGroupDelete(userName,groupName);
		
		socketConnection.print(groupDelete);
	}

	public void sendHistoryMessage(String with, String numMsgs){
		if(!connectionActive()){
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		Message m = Message.makeHistoryMessage(this.userName, with, numMsgs);
		socketConnection.print(m);
	}

	public void sendSearchMessage(String type, String with, String startTime, String endTime){
		if(!connectionActive()){
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		Message m = Message.makeSearchMessage(this.userName, type, with, startTime, endTime);
		socketConnection.print(m);
	}

	public void sendParentalControlMessage(String onOff) {
		if(!connectionActive()){
			throw new IllegalOperationException("Cannot send a message if you are not connected to a server!\n");
		}
		Message m = Message.makeParentalControlMessage(this.userName, onOff);
		socketConnection.print(m);
	}
	...
	private boolean login() {
		...
		Message loginMessage = Message.makeLoginMessage(userName,this.password);
	...
	private boolean checkForCrudOperation(String option) {
		// Now log in using this name.
		switch(option) {
	...
	private boolean updateUser() {
		...
		Message loginMessage = Message.makeRegisterMessage(userName,this.password);
