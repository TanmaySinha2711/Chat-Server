...
public class ClientDupingTest {
	...
	public final void setUp() {
		...
		try {
			...
			serverSocket.socket().bind(new InetSocketAddress(4540));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		clientTerminal = new IMConnection("127.0.0.1", 4540, "agency", "password");
	 ...
	 */
	@Test
	void testRunUserDupingMessage() {
		SocketChannel socket;
		try {
			socket = serverSocket.accept();
			ClientRunnable runnable = new ClientRunnable(socket);
			clientTerminal.sendDupingMessage("avik");
			clientTerminal.disconnect();
	 ...
	 */
	@Test
	void testRunGroupDupingMessage() {
		SocketChannel socket;
		try {
			socket = serverSocket.accept();
			ClientRunnable runnable = new ClientRunnable(socket);
			clientTerminal.sendDupingMessage("team");
			clientTerminal.disconnect();
